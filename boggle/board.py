...
class Board:
        ...
        for node_id in nodes:
            ...
            while len(queue) > 0:
                ...
                if node["letter"] == chars[len(path)]:
                    if node_id in path:
                        continue  # skip any node in path

                    path.append(node_id)
                    if len(word) == len(path):
                        return True

                    neighbors = self.GetNeighbors(node_id)
                ...
                if len(neighbors) == 0 and len(path) > 0:
                    ...
                    if len(path) > 0:
                        ...
                        queue.remove(bad_node)

        return False

    def GetNeighbors(self, node):
        if node in self.nodes:
            return list(self.nodes[node]["neighbors"].keys())
        else:
            return None
    ...
    def FileToBoard(self, board):
        ...
        for i in range(len(lines)):
            ...
            for j in range(len(chars)):
                if self.__IsALetter(chars[j]):
                    self.AddNode((i, j), chars[j])

    ## private methods ###
    def __AddNeighbors(self, node):
    ...
    def __IsALetter(self, letter):
        if self.LETTER_REGEX.match(letter) and len(letter) == 1:
            return True
        else:
            return False
